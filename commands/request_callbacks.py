...
async def handle_request_callback(update, context: CallbackContext):
    ...
    try:
        ...
        if callback_data.startswith("movie_nav_"):
            ...
            already_text = "✅ This content is already in your library\\!"
...
async def handle_movie_navigation(query, callback_data):
    ...
    keyboard = request_manager.create_movie_keyboard(movie, new_index, len(results), search_id, already_exists)
...
async def handle_tv_navigation(query, callback_data):
    ...
    keyboard = request_manager.create_tv_keyboard(show, new_index, len(results), search_id)
...
async def handle_add_movie(query, callback_data):
    ...
    if len(root_folders) == 1 and len(quality_profiles) == 1:
        ...
        success, error = await add_movie_to_radarr(movie, root_folder, quality_profile)
        if success:
            title = movie.get("title", "Unknown")
            success_text = f"✅ *{escape_md(title)}* has been added to Radarr\\!"
            if query.message.photo:
...
async def handle_add_tv(query, callback_data):
    ...
    if len(root_folders) == 1 and len(quality_profiles) == 1:
        ...
        success, error = await add_tv_to_sonarr(show, root_folder, quality_profile)
        if success:
            title = show.get("name", "Unknown")
            success_text = f"✅ *{escape_md(title)}* has been added to Sonarr\\!"
            if query.message.photo:
                await query.edit_message_caption(caption=success_text, parse_mode=ParseMode.MARKDOWN_V2)
            else:
                await query.edit_message_text(success_text, parse_mode=ParseMode.MARKDOWN_V2)
...
async def handle_quality_profile_selection(query, callback_data):
    ...
    if media_type == "movie":
        ...
        success, error = await add_movie_to_radarr(movie, root_folder, selected_profile)
        title = movie.get("title", "Unknown")
    else:
        show = media_data["show"]
        success, error = await add_tv_to_sonarr(show, root_folder, selected_profile)
        title = show.get("name", "Unknown")
    
    if success:
        app_name = "Radarr" if media_type == "movie" else "Sonarr"
        success_text = f"✅ *{escape_md(title)}* has been added to {app_name}\\!"
        if query.message.photo:
            await query.edit_message_caption(caption=success_text, parse_mode=ParseMode.MARKDOWN_V2)
        else:
            await query.edit_message_text(success_text, parse_mode=ParseMode.MARKDOWN_V2)
...
async def add_movie_to_radarr(movie, root_folder, quality_profile):
    """Add movie to Radarr"""
    ...
    try:
        ...
        async with AsyncClient() as client:
            ...
            for api_version in ["v3", "v2", "v1"]:
                ...
                try:
                    ...
                    if resp.status_code in [200, 201]:
                        logger.info("✅ Movie added to Radarr using API %s: %s", api_version, title)
                        return True, None
            ...
            return False, "All Radarr API versions failed"
            
    except Exception as e:
        logger.error("❌ Failed to add movie to Radarr: %s", e)
        return False, str(e)

async def add_tv_to_sonarr(show, root_folder, quality_profile):
    """Add TV series to Sonarr with only latest season monitored"""
    ...
    try:
        ...
        if not tvdb_id:
            return False, "Could not find TVDB ID for this series"
        ...
        async with AsyncClient() as client:
            ...
            for api_version in ["v3", "v2", "v1"]:
                ...
                try:
                    ...
                    if resp.status_code in [200, 201]:
                        logger.info("✅ Series added to Sonarr using API %s: %s (latest season only)", api_version, title)
                        return True, None
                    elif resp.status_code == 404:
                        continue
                    else:
            ...
            return False, "All Sonarr API versions failed"
