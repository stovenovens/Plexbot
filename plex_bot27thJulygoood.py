...
# Log bot topic configuration
logger.info("📱 Bot topic ID configured: %s", BOT_TOPIC_ID)

# --- Helper function for sending messages to bot topic ---
async def send_to_bot_topic(bot: Bot, message: str, parse_mode=None):
    ...
    try:
        ...
        logger.info("✅ Message sent to bot topic successfully")
        ...
        try:
            ...
            logger.info("✅ Message sent to main group as fallback")
...
async def scheduled_wake(bot: Bot):
    ...
    # Log system status
    logger.info("🔍 Pre-wake check - MAC: %s, Broadcast: %s, Group ID: %s, Topic ID: %s", 
                PLEX_MAC, PLEX_BROADCAST_IP, GROUP_CHAT_ID, BOT_TOPIC_ID)
...
async def welcome_command(update, context: CallbackContext):
    """Send a welcome message to the bot topic explaining available commands"""
    user_id = update.effective_user.id
    if user_id not in OFF_USER_IDS:  # Only authorized users can send welcome
        return await update.message.reply_text("❌ Not authorized.")
    
    try:
        welcome_msg = "🤖 *Welcome to the Plex Bot Topic\\!*\n\n"
        welcome_msg += "This topic is dedicated to bot commands and server notifications\\. Here's what I can do:\n\n"
        welcome_msg += "*🎬 Quick Commands:*\n"
        welcome_msg += "\\- `/np` \\- See who's watching what right now\n"
        welcome_msg += "\\- `/on` \\- Wake up the Plex server\n"
        welcome_msg += "\\- `/stats` \\- Weekly viewing leaderboard\n"
        welcome_msg += "\\- `/upcoming` \\- This week's new episodes & movies\n"
        welcome_msg += "\\- `/hot` \\- Trending movies & shows\n\n"
        welcome_msg += "*🔔 Auto Notifications:*\n"
        welcome_msg += "\\- Server wake\\-ups \\(weekdays 4:30pm, weekends 10am\\)\n"
        welcome_msg += "\\- System status updates\n"
        welcome_msg += "\\- Error alerts\n\n"
        welcome_msg += "*💡 Pro Tip:* Use `/info` for the complete command list\\!"
        
        await send_to_bot_topic(context.bot, welcome_msg, parse_mode=ParseMode.MARKDOWN_V2)
        await update.message.reply_text("✅ Welcome message sent to bot topic!")
...
if __name__ == "__main__":
    ...
    async def on_startup(app):
        ...
        # Add jobs - automated wake-ups will be sent to bot topic
        scheduler.add_job(
            scheduled_wake, 
    ...
    app.add_handler(CommandHandler("welcome", welcome_command))  # New welcome command
