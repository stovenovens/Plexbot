...
# --- Scheduled tasks ---
async def scheduled_send(bot: Bot):
    logger.info("✅ Weekly hot job triggered at %s", datetime.now(MELBOURNE_TZ))
    try:
        text = await format_message()
        await bot.send_message(chat_id=GROUP_CHAT_ID, text=text, parse_mode=ParseMode.MARKDOWN_V2)
        logger.info("✅ Weekly hot message sent successfully")
    except Exception as e:
        logger.error("❌ Weekly hot message failed: %s", e)
...
if __name__ == "__main__":
    ...
    async def on_startup(app):
        ...
        # Add jobs - scheduler timezone is set globally
        scheduler.add_job(
            scheduled_send, 
